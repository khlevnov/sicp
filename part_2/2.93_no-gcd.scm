...
(define (install-dense-polynomial-package)
    ...
    (define (adjoin-term terms)
        (lambda (term)
            (if (= (order term) (length terms))
                (cons (coeff term) terms)
                ((adjoin-term (cons 0 terms)) term))))
...
(define (install-polynomial-package)
    ...
    (define (to-variable v p)
        ...
        (define (invert p)
            (define (invert-term term)
                (let ((new-variable (variable p))
                      (old-variable (variable (coeff term)))
                      (order-by-old-variable
                          (order (first-term (term-list (coeff term)))))
                      (coeff-by-old-variable
                          (coeff (first-term (term-list (coeff term)))))
                      (order-by-new-variable
                          (order term)))
                     (make-term
                         (make-polynomial
                             new-variable
                             ((adjoin-term (the-empty-termlist))
                                (make-term
                                    ...
                                    order-by-new-variable)))
                         order-by-old-variable)))
            (define (invert-iter terms inverted)
                (if (empty-termlist? terms)
                    inverted
                    (invert-iter
                        (rest-terms terms)
                        ((adjoin-term inverted)
                            (invert-term
                                (first-term terms))))))
            (make-polynomial v
                (invert-iter
    ...
    (put 'make-polynomial 'polynomial (lambda (variable term-list)
...
(define (make-polynomial variable term-list)
    ((get 'make-polynomial 'polynomial) variable term-list))
(define (to-variable variable term-list)
    ((apply-generic 'to-variable term-list) variable))
...
(define numer1 (make-sparse-polynomial 'x (list '(1 1) '(1 0))))
(define denom1 (make-sparse-polynomial 'x (list '(1 3) '(1 0))))
(define rf1 (make-rational numer1 denom1))

(define numer2 (make-sparse-polynomial 'x (list '(1 1))))
(define denom2 (make-sparse-polynomial 'x (list '(1 2) '(-1 0))))
(define rf2 (make-rational numer2 denom2))
